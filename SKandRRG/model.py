...
class NADE(BaseModel):
    def __init__(self, n, hidden_dim, device="cpu", z2=False, *args, **kwargs):
        super().__init__()
...
# MADE (no hidden layers, fully visible sigmoid belief network)
# class MADE(BaseModel):
#     def __init__(self, n, device="cpu", z2=False, *args, **kwargs):
#         super().__init__()
...
class MADE(BaseModel):
    def __init__(self, n, num_channels, device="cpu", z2=False, *args, **kwargs):
        super().__init__()
...
class MeanField(BaseModel):
    def __init__(self, n, *args, **kwargs):
        super().__init__()

        self.register_parameter("theta", nn.Parameter(1e-3 * torch.randn(n)))
    ...
    @torch.no_grad()
    def sample(self, batch_size):
        p = torch.sigmoid(self.theta).expand(batch_size, self.n)
        return torch.bernoulli(p)

    def forward(self, x, eps=1e-8):
        p = torch.sigmoid(self.theta).expand(x.size())
